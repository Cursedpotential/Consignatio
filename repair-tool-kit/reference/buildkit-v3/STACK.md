...
| PDF text (primary) | **poppler** `pdftotext` | **[sandbox-verified]** | Decoded all 33 ZapfDingbats glyphs — but in a Linux sandbox, **not** with the Windows binary casekit will ship. Phase 0 re-runs the bake-off against the pinned binary. |
...
| Install root | **UNRESOLVED — see OPEN-DECISIONS OD-1** (`D:\case_apps` vs `E:`) | — | STACK and the Consignatio AGENTS.md storage rule disagree |
...
- **mutool / MuPDF** — **rejected for text extraction; retained only as a
  frozen test fixture** (its known-bad output drives the Phase 3 escalation
  test). Not a shipped engine, not required to be installed. On the real
