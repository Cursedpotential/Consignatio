...
both `casekit.exe` and `casekitw.exe`; launching `casekitw.exe` via a
`.lnk` shortcut and via `cmd /c start` produces **no visible console flash**
(a registry verb is Phase 7 and is NOT required here); and once any external
binary is invoked, its child process shows no console either; `duckdb` loads the
`webbed` extension successfully.
Also: **re-run the 33-glyph bake-off against the pinned Windows poppler** that
will live under the install root, and record its version and full path. The
central empirical claim in `FINDINGS.md` was verified in a Linux sandbox, not
with the binary that will ship. One command; converts it from
sandbox-verified to target-verified.
**Gotchas:** A-6 (parent *and* child processes)
...
check passes. Escalation is proven against a **frozen golden fixture** of
known-bad output — mutool's recorded result for this file, `I` (U+0049) for
every ZapfDingbats glyph, 0 non-ASCII — which must FAIL `symbol_glyph_count`
and trigger automatic escalation to the next engine in rank. The fixture is a
test artifact; mutool is **not** a shipped engine and need not be installed. The routing
...
## Phase 5b — ZIP containers (unwrap, recurse, repair)
...
## Phase 5c — JSON extraction (Facebook / Instagram / generic)
...
Screenshots (OCR), call logs, DOCX/OOXML (which are zips — Phase 5b does the
