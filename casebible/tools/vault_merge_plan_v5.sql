-- Byline: Claude Code · Opus 5 · 2026-09-15 08:05 EDT (session propria-79)
-- Vault step 1 (collapse sources into one root) — MERGE PLAN v5 DRY RUN, analysis tables only. NO B2 writes.
...
drop table if exists raw_duck.vault_mounts_v5;
create table raw_duck.vault_mounts_v5 as
select m.*,
       case when m.target ~* '(^|/)(\.review_hold|_swept|_duplicates|_?to_be_deleted)(/|$)' then 'dropped: trunk cleanup folder'
...
create temp table mm as select * from raw_duck.vault_mounts_v5 where decision like 'used%';
create index on mm (src);

create temp table place as
select distinct on (o.src, o.rel)
       o.md5, o.size, o.src, o.rel, o.unit_root, o.unit_class, mm.mp, mm.trunk_head, mm.f, mm.decision,
...
drop table if exists raw_duck.vault_place_v5;
create table raw_duck.vault_place_v5 as
select p.md5, p.size, p.src, p.rel, p.unit_root, p.unit_class, p.mp, p.trunk_head, p.decision, uf.vunit,
...
from (select vpath1, md5, size, count(*) n, bool_or(src = 'onedrive/Case Bible') trunk
      from raw_duck.vault_place_v5 where unit_root is null group by 1, 2, 3) t;
alter table raw_duck.vault_place_v5 add column vpath text, add column new_depth int;
update raw_duck.vault_place_v5 v set vpath = case when v.unit_root is null and fr.rk > 1
...
update raw_duck.vault_place_v5 set vpath = vpath1 where vpath is null;
update raw_duck.vault_place_v5 set new_depth = cardinality(string_to_array(vpath, '/'));
create index on raw_duck.vault_place_v5 (vpath);
commit;

\echo === mount decisions per source
select src, decision, count(*) mounts, sum(support) support, sum(source_files) source_files
from raw_duck.vault_mounts_v5 group by 1, 2 order by 1, 2;
select decision, count(*) mounts, sum(source_files) source_files from raw_duck.vault_mounts_v5 group by 1 order by 1;
...
from raw_duck.vault_mounts_v5 where decision like 'used%' order by source_files desc limit 30;
...
from raw_duck.vault_mounts_v5 where decision like 'dropped%' order by source_files desc limit 15;

\echo === guards (all MUST be 0)
select 'trunk rows moved' k, count(*) from raw_duck.vault_place_v5 where src = 'onedrive/Case Bible' and vpath <> rel
union all select 'rows more than 1 level deeper', count(*) from raw_duck.vault_place_v5 where new_depth > old_depth + 1
union all select 'rows +1 deeper without a +1 mount', count(*) from raw_duck.vault_place_v5 where new_depth = old_depth + 1 and coalesce(decision, '') <> 'used: +1 level'
union all select 'rows moved into a trunk cleanup folder', count(*) from raw_duck.vault_place_v5 where mp is not null and vpath ~* '(^|/)(\.review_hold|_swept|_duplicates|_?to_be_deleted)(/|$)' and rel !~* '(^|/)(\.review_hold|_swept|_duplicates|_?to_be_deleted)(/|$)'
union all select 'units split across landing roots', count(*) from (select src, unit_root from raw_duck.vault_place_v5 where unit_root is not null group by 1, 2 having count(distinct vunit) > 1) t
union all select 'paths with different contents', count(*) from (select vpath from raw_duck.vault_place_v5 group by 1 having count(distinct (md5, size)) > 1) t;
...
from raw_duck.vault_place_v5 group by 1 order by 2 desc;
...
select count(*) occurrences, count(distinct vpath) paths,
       (select count(*) from (select distinct vpath, md5, size from raw_duck.vault_place_v5) d) objects,
       (select round(sum(size) / 1e9, 1) from (select distinct vpath, md5, size from raw_duck.vault_place_v5) d) gb_if_copied
from raw_duck.vault_place_v5;
...
from raw_duck.vault_place_v5 group by 1 order by 2 desc limit 40;

\echo === examples flagged earlier
select src, left(rel, 90) rel, left(vpath, 90) vpath from raw_duck.vault_place_v5
