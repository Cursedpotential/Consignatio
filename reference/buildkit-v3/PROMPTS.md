...
Create:
  ...
  internal/config/config.go  - TOML config load, install root from OPEN-DECISIONS OD-1 (UNRESOLVED - ask, do not guess)
...
EXIT CRITERION: `go build ./...` succeeds; both binaries produced; launching
casekitw.exe --version via a .lnk shortcut AND via `cmd /c start` produces no
visible console window (a registry verb is Phase 7 — do NOT create one here);
any child process spawned shows no console either (GOTCHAS A-6 — needs
SysProcAttr HideWindow on every exec.Cmd, stdlib, no new dependency);
...
EXIT CRITERION: routing the reference PDF selects poppler and passes.
Escalation is proven against a FROZEN GOLDEN FIXTURE of mutool's known-bad
output for this file (every ZapfDingbats glyph as "I" U+0049, 0 non-ASCII),
which must FAIL symbol_glyph_count and auto-escalate. mutool is a test fixture,
NOT a shipped engine - do not install it and do not add it to the registry.

Verify with: casekit extract --dry-run docs/reference/*.pdf
             go test ./internal/route -run TestEscalation
```
